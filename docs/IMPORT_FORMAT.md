...
## 1. Parser 契约与阶段 5 调用边界

Parser 只实现本地临时文件路径到 Canonical Chat Schema 的确定性解析：识别格式、读取原始字节、计算 SHA-256、解析、统一验证并返回安全错误和统计。Parser 自身不修改源文件、不访问网络、不写数据库、不上传文件、不清洗正文。阶段 5 API 在外层完成显式上传、限制、Cleaner 编排和事务入库；WeFlow 仍不支持。
...
- 阶段 5 已提供显式单文件上传、重复 SHA-256 查询、Cleaner 编排和事务写入；仍没有目录扫描或压缩包支持。
- Cleaner 支持确定性规范化、可选脱敏、非破坏性排除和 AnalysisUnit；不读取附件，也不自动探测编码。
- 文件大小、会话/参与者/消息数量、单条正文和 metadata 上限由阶段 5 Settings 配置，并在入库前验证。
