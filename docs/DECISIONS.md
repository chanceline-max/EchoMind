...
## ADR-026：项目采用 Apache License 2.0

- 状态：Accepted
- 日期：2026-07-18
- 决策：EchoMind 0.1.0 及后续项目贡献采用 Apache License 2.0（SPDX：`Apache-2.0`）；版权标注为 `Copyright 2026 杨锦辰`。标准正文放在根目录 `LICENSE`，项目归属放在 `NOTICE`，第三方依赖继续适用其各自许可证。
- 原因：项目所有者已明确选择包含专利授权条款的标准 OSI 许可证，并确认版权持有人和年份。包元数据、README 和贡献流程需要使用同一标识。
- 后果：贡献者必须有权提交内容，并同意贡献按 Apache-2.0 许可；该决定不改变第三方依赖许可证，也不表示项目已完成生产安全或法律合规认证。

## ADR-027：中文界面与 candidate-extraction-1.1 输出语言边界

- 状态：Accepted
- 日期：2026-07-19
- 决策：前端把 Insight 状态/类型、Evidence 状态/角色、修订动作、Profile 状态和已知原因码映射为简体中文显示；API、数据库、JSON、受控枚举和版本字段继续使用稳定英文值。
- 决策：Prompt 与抽取版本升级为 `candidate-extraction-1.1`。1.1 保持 1.0 的窗口、证据、安全和候选验证规则，只新增输出语言约束：`title`、`statement`、`reasoning_basis` 和 `alternative_explanations` 使用简体中文；`category` 等受控枚举和局部 ID 不翻译。
- 决策：1.0 历史 Insight 不自动翻译、不覆盖 title/statement，也不重写指纹或 Revision。用户可在既有审核入口显式编辑旧候选；之后重新分析产生的 1.1 候选按新版本独立持久化。
- 原因：界面直接显示英文内部值且英文 Prompt 未规定输出语言，导致中文聊天产生英文候选。显示层映射能够在不破坏 API 契约的情况下中文化；版本升级避免无痕改变模型行为。
- 后果：已有英文自由文本仍保持英文，且 1.1 重新分析可能与 1.0 产生语义相近但指纹不同的候选；当前不引入自动翻译服务、批量覆盖或额外模型调用。

## ADR-028：高置信候选使用显式批量确认而非自动确认

- 状态：Accepted
- 日期：2026-07-23
- 决策：最终置信度严格高于 0.5、状态为 proposed、证据状态为 valid 且类型为 fact/preference/pattern/change 的候选可进入批量确认队列。inference、hypothesis、contradiction、非 valid 证据和小于等于 0.5 的候选必须逐条审核。
- 决策：系统不根据分数自动修改 status。用户必须显式点击并确认当前批次数量；请求最多包含 50 个明确 Insight ID 和 expected revision。后端在一个事务内重新检查全部资格，每条成功确认都写入独立的 local_user Revision，任一失败整批回滚。
