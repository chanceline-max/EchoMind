...
- 导入任务支持 pending、running、completed、failed 状态和阶段统计；取消和复杂调度不属于 MVP。
...
- 正式导出默认只包含 confirmed Insight；用户显式启用后可以纳入达到阈值的 proposed，但必须逐条显示“未确认”。
...
| 大文件导致 SQLite 锁和内存压力 | 导入慢或失败 | 流式解析、批量事务、单 worker、检查点 | 限制值需阶段 3/5 性能与安全测试确定 |
