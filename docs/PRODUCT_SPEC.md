...
6. 系统仅使用 confirmed Insight 生成综合人物分析，并输出 Markdown 和 JSON EchoProfile。
...
- **EchoProfile**：由 confirmed Insight 综合而成的版本化人物分析快照，不是 Insight 列表、正式测评或永久真相。
...
- 医学或心理诊断、风险预测，或把 MBTI/Big Five 当作决定性人格结论。
...
- 矛盾不被自动抹平，应以明确关系展示。
- 支持由用户显式触发的高置信批量确认，以减少逐条操作；仅限置信度严格高于 50%、证据有效的 fact/preference/pattern/change。inference、hypothesis、contradiction 和其他不满足条件的候选不能进入批量范围。
- 批量确认不是自动确认：提交前展示数量，后端重新验证乐观锁和资格，任一条失败则整批回滚，每条成功项保留独立 Revision。

### Profile

- Profile 明确生成时间、覆盖范围、数据数量、版本和限制。
- EchoProfile 2.0 将已确认 Insight 跨主题综合为人物性格、思维、决策、价值、关系、压力、优势、成长、矛盾和变化，不以信息卡片堆叠作为最终正文。
- Markdown 与 JSON 在综合摘要、叙事章节、人格框架和不确定性上语义等价。
...
- 2.0 正文不显示 Evidence 编号、消息链接或证据索引，但内部证据链和 source manifest 必须保留，用于 stale、失效传播和追溯审计。
- 2.0 必须包含 Big Five 五维和 MBTI 四组偏好的辅助映射；二者都要显示参考强度和限制，不得声称完成标准化问卷或用于决定职业、关系和人生选择。
- 默认 Mock Provider 只返回“信息不足”的确定性结构；远程综合必须由用户当前请求显式授权。
...
| MVP-A10 | 对同一冻结输入生成的 Markdown/JSON 通过对应版本 schema；1.0 的 Insight/Evidence 引用一致，2.0 的综合章节与 Big Five/MBTI 一致且不公开消息/证据 ID；相同来源和选项幂等复用 | Profile 单元测试、schema/契约测试 |
| MVP-A11 | 每个 Profile 结论的内部来源仍可沿 Insight→Evidence→Message→SourceFile 审计；2.0 隐藏公开引用但不删除 manifest，断链会使 Snapshot stale/source unavailable 且失效内容不能作为新档案的有效事实 | 数据库集成测试、API 集成测试、Playwright |
