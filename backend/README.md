...
阶段 8 的 FastAPI、数据库、Parser、Cleaning、同步导入、LLM Provider、候选 Insight 抽取和确定性置信度工程。`src/echomind/confidence/` 提供显式请求、只读特征加载、因子、公式、最低规则、输入指纹、固定解释、短事务持久化和安全报告。
...
当前仍不包含分析、Provider 或置信度 HTTP API、前端模型/Insight 审核页面、用户置信度 override、Profile、Local 模型运行或远程自动调用。安装、启动、公式、Provider 配置和完整检查命令见仓库根目录的 `README.md`。
