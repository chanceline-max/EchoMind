...
export function InsightDetailPage() {
  ...
  if (detail.isPending) return <main className="content-page"><p>正在读取 Insight…</p></main>;
  if (detail.isError || !insight) return <main className="content-page"><p className="error-box">无法读取 Insight。</p></main>;

  return (
    <main className="content-page review-page">
      <Link className="back-link" to="/insights">← 返回 Insight 列表</Link>
      <div className="insight-detail-heading">
        <div><p className="eyebrow">{insight.category} · {insight.insight_type}</p><h1>{insight.title}</h1></div>
        <div className={`evidence-seal seal-${insight.evidence_state}`}><small>最终置信度</small><strong>{percent(insight.confidence)}</strong><span>{insight.evidence_state}</span></div>
      </div>

      {conflict && <div className="conflict-box" role="alert"><strong>此 Insight 已在其他页面被修改。</strong><span>当前内容没有被覆盖，请重新加载后再提交。</span><button onClick={() => { mutation.reset(); void detail.refetch(); void revisions.refetch(); }}>重新加载</button></div>}
      {mutation.isError && !conflict && <p className="error-box" role="alert">审核操作失败：{mutation.error.message}</p>}

      <section className="claim-sheet">
        <div className="claim-meta"><span className={`status-chip status-${insight.status}`}>{insight.status}</span><span>revision {insight.revision_number}</span><span>模型自评 {percent(insight.model_confidence)}</span><span>规则版本 {insight.confidence_version}</span></div>
      ...
      <section className="review-actions" aria-label="审核操作">
        ...
        {insight.allowed_actions.includes("confirm") && <button className="primary-button" disabled={mutation.isPending} onClick={() => mutation.mutate({ kind: "confirm", body: { expected_revision: expected } })}>确认 Insight</button>}
        {insight.allowed_actions.includes("reject") && <button className="danger-button" disabled={mutation.isPending} onClick={() => { const note = window.prompt("请填写驳回原因（不会删除 Evidence 或历史）"); if (note) mutation.mutate({ kind: "reject", body: { expected_revision: expected, note } }); }}>驳回</button>}
        {insight.allowed_actions.includes("restore_to_proposed") && <button onClick={() => mutation.mutate({ kind: "restore", body: { expected_revision: expected, target_status: "proposed" } })}>恢复为 proposed</button>}
        ...
        {insight.allowed_actions.includes("supersede") && <button onClick={() => setShowSupersede(!showSupersede)}>用其他 Insight 替代</button>}
      </section>
      ...
      {showEditor && <form className="panel review-form" onSubmit={submitEdit}>
        ...
        <div className="split-fields"><label>分类<input name="category" defaultValue={insight.category} required /></label><label>类型<select name="insight_type" defaultValue={insight.insight_type}>{["fact", "preference", "pattern", "inference", "hypothesis", "contradiction", "change"].map((value) => <option key={value}>{value}</option>)}</select></label></div>
        ...
        <button className="primary-button" disabled={mutation.isPending}>保存 revision {expected + 1}</button>
      </form>}
      ...
      {showSupersede && <form className="panel review-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); mutation.mutate({ kind: "supersede", body: { expected_revision: expected, replacement_insight_id: data.get("replacement"), note: data.get("note") || null } }); }}>
        <h2>替代当前 Insight</h2><p>当前记录和 Evidence 都会保留，只把状态标记为 superseded。</p>
        <label>替代 Insight ID<input name="replacement" required /></label><label>说明<textarea name="note" rows={2} /></label><button>确认替代</button>
      </form>}

      <section className="review-section"><div className="section-heading"><p className="eyebrow">Traceability</p><h2>证据链</h2><span>{insight.valid_evidence_count}/{insight.evidence_count} 有效</span></div>
        <div className="evidence-list">{insight.evidence.map((evidence) => <article key={evidence.evidence_id} className={`evidence-card${evidence.is_valid ? "" : " is-invalid"}`}><header><strong>{evidence.sender_role === "PROFILE_OWNER" ? "Profile Owner" : "Other participant"}</strong><span>{evidence.stance}</span><span>{evidence.is_valid ? "有效" : "已失效"}</span></header><blockquote>{evidence.excerpt}</blockquote>{evidence.invalidation_reasons.length > 0 && <p className="invalidation-line">失效原因：{evidence.invalidation_reasons.join("、")}</p>}<Link to={evidence.message_link}>查看原消息 →</Link></article>)}</div>
      </section>

      <section className="review-section"><div className="section-heading"><p className="eyebrow">Append-only</p><h2>Revision 历史</h2><span>{revisions.data?.total ?? 0} 条</span></div>
        {revisions.isPending && <p>正在读取修订…</p>}{revisions.isError && <p className="error-box">无法读取修订历史。</p>}
        <ol className="revision-list">{revisions.data?.items.map((revision) => <li key={revision.id}><div><strong>r{revision.revision_number} · {revision.action}</strong><span>{revision.actor_type === "system" ? "系统传播" : "本地用户"}</span></div><time>{new Date(revision.created_at).toLocaleString()}</time>{revision.note && <p>{revision.note}</p>}<small>变更：{Object.keys(revision.changed_fields_json).join("、") || "状态已复核"}</small></li>)}</ol>
      </section>
    </main>
  );
}
