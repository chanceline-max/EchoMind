...
export function ProfileDetailPage() {
  ...
  return <main className="content-page profile-detail-page">
    ...
    <section className="profile-meta-strip"><div><span>结构版本</span><strong>{profile.schema_version}</strong></div><div><span>策略</span><strong>{selectionPolicyLabel(profile.document.metadata.selection_policy)}</strong></div><div><span>证据模式</span><strong>{evidenceModeLabel(profile.evidence_mode)}</strong></div><div><span>洞察 / 证据</span><strong>{profile.insight_count} / {profile.evidence_count}</strong></div></section>
