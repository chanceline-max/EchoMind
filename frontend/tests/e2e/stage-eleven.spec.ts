...
test("completes the user-reachable MVP loop from an empty database", async ({ page }) => {
  ...
  await expect(page.getByText("离线模拟模型", { exact: true })).toBeVisible();
  ...
  await expect(page.locator(".analysis-metrics")).toContainText("1 新洞察");
  await expect(page.locator(".analysis-metrics")).toContainText("1 已评分");
  await page.getByRole("link", { name: "查看生成的洞察" }).click();
  ...
  await page.getByRole("link", { name: "认知档案", exact: true }).click();
