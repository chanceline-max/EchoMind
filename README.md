...
当前已完成 **阶段 8：可解释、可版本化的最终置信度计算与幂等重算**。阶段 7 候选抽取现在会持久化 `explicit_self_report`；内部 `echomind.confidence` 模块只读取调用方明确指定的 Insight 及本地证据统计，以 `confidence-1.0` 机械规则计算因子、类型最低条件、相反证据惩罚、类型上限和固定模板解释。它不调用模型或网络，也没有评分 HTTP API。当前仍没有 Insight 审核 UI、用户 override 或 Profile 生成。
...
阶段 8 已实现内部 `confidence-1.0`、Evidence 状态重算、最低规则、结构化因子、安全解释和幂等持久化；阶段 5 导入闭环、阶段 6 Provider 和阶段 7 Extraction 回归保持可用。真实 WeFlow 仍未支持；当前也没有分析/Provider/评分 API、模型设置或 Insight 审核页面、用户 override、跨窗口语义合并、EchoProfile、账户权限、Docker 或 CI。
