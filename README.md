...
当前已完成 **阶段 10：EchoProfile 结构化文档、确定性导出与不可变快照**。本地 Profile 页面只选择 confirmed Insight，以同一个严格 `EchoProfileDocument` 生成 Markdown 和 JSON；稳定的 I/E 引用、来源/生成/文档指纹、幂等快照和动态 stale 标记共同保留证据链。当前仍没有 Profile 编辑/删除、proposed Profile、PDF/Word、云分享、用户自定义 confidence、批量审核或多用户权限。
...
阶段 10 已实现 confirmed-only EchoProfile API/UI、共享结构化文档、确定性 Markdown/JSON、不可变幂等快照、显式导出和动态 stale 检测；阶段 5 导入闭环及阶段 6–9 回归保持可用。真实 WeFlow 仍未支持；当前也没有分析/Provider/独立评分 API、模型设置、批量审核、用户 confidence override、跨窗口语义合并、Profile 编辑/删除、proposed Profile、PDF/Word、云分享、账户权限、Docker 或 CI。
