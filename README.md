...
**让散落在对话里的经历、偏好与思想变化，重新变得可理解。**

我们每天都在聊天中留下关于自己的线索：反复出现的困惑、长期坚持的偏好、逐渐改变的观点，以及一些当时没有意识到的矛盾与模式。

聊天软件擅长保存消息，却很少帮助我们理解这些内容如何在时间中连接起来。EchoMind 想做的，是把长期对话转化为一份持续演化、可以追溯、可以质疑、也可以由本人修订的个人认知地图。

它不会简单地给人贴上人格标签，也不会把 AI 的猜测包装成事实。每项重要判断都应带有原始证据、时间信息、不确定性和其他可能解释，最终由用户决定接受、修改还是驳回。

[GitHub 仓库](https://github.com/chanceline-max/EchoMind) · [v0.1.0 Release](https://github.com/chanceline-max/EchoMind/releases/tag/v0.1.0) · [Apache-2.0](LICENSE) · [MVP 审计报告](docs/MVP_AUDIT.md)

## 为什么需要 EchoMind

长期聊天记录不只是消息列表。它们也记录了一个人在不同关系、情境和时间里的选择、表达和变化。但要从数千条消息中重新找到这些线索，普通搜索和一次性摘要都不够。

EchoMind 关注的不是“AI 能否定义你”，而是三个更克制的问题：

1. 一项判断来自哪些原始消息？
2. 它是明确事实、稳定偏好、重复模式，还是仍待验证的推断？
3. 当新证据出现或用户不同意时，它能否被修正，而不是成为永久标签？
...
在这个过程中，EchoMind 会同时保留不可被清洗覆盖的 `raw_content` 和可重建的 `normalized_content`。最终的 `EchoProfile` 不是永久真相，而是由当前已确认 Insight 生成的不可变快照；来源后来发生变化时，旧快照会显示 stale 或证据失效，不会被静默改写。

用户最终可以获得：

- **Insight**：区分事实、偏好、模式、推断、假设、矛盾和变化的候选认知单元。
- **Evidence**：能够回到具体消息和来源文件的支持、反对或上下文证据。
- **Confidence**：说明当前证据支撑强度的版本化机械分数，而不是心理学概率。
- **Revision**：记录用户确认、编辑、驳回、恢复和替代判断的修订历史。
- **EchoProfile**：把 confirmed Insight 综合为人物性格、思维、决策、关系与变化的完整档案，并可导出为 Markdown 或 JSON。

### EchoProfile 2.0：从信息列表到人物理解

当前开发版本不再把证据编号和 Insight 卡片直接堆成“认知档案”。它会在内部保留完整证据链，用这些来源判断档案是否过期；面向用户的正文则跨主题综合为一份连贯的人物分析，包括：

- 核心人格题名与整体概括；
- 思考方式、决策模式、价值驱动、关系模式和压力反应；
- 可能的优势、成长方向、内在矛盾与时间变化；
- Big Five 五维参考与 MBTI 四组偏好参考；
- 信息不足、场景偏差和其他可能解释。

Big Five 和 MBTI 只是帮助理解的两套语言，不是正式问卷结果，也不会替代完整分析或决定一个人“是什么类型”。默认离线 Mock Provider 只返回信息不足的结构，不会伪造个性化判断；真实综合分析仍需要用户显式配置并授权远程 Provider。

## 一个具体例子

假设一段长期对话中，在不同时间点反复出现了“更喜欢独立完成需要深度思考的任务”这样的表达。

EchoMind 的目标不是据此宣布“你属于某种人格”，而是形成一条等待审核的偏好或模式：展示它引用了哪些消息、这些消息发生在什么时候、是否存在相反证据、为什么得到当前置信度，以及还有哪些其他解释。用户可以确认它，也可以改写成更准确的陈述，或者直接驳回。

> v0.1.0 默认使用完全离线的 Mock Provider，它用于验证完整工作流，不会根据真实聊天正文自动生成候选。要获得真实模型候选，用户必须显式配置兼容的远程 Provider，并对每次分析逐次授权。

## 它与一次性聊天总结有什么不同

| 一次性聊天总结 | EchoMind |
|---|---|
| 生成一段当前回答 | 维护可持续修订、可版本化的认知档案 |
| 来源往往难以逐项核对 | 重要判断必须绑定可追溯 Evidence |
| 容易把概括写成确定结论 | 明确区分 fact、preference、pattern、inference 和 hypothesis |
| 容易压平时间变化与冲突 | 保留 contradiction、change、有效期和相反证据 |
| 用户主要负责阅读结果 | 用户可以确认、编辑、驳回、恢复和替代判断 |
| 通常以一次模型调用为中心 | 默认本地优先，远程调用必须显式启用并逐次同意 |

## 设计底线

- **证据优先于表达效果**：没有明确自述证据不能生成 fact，单条消息不能直接生成 pattern。
- **不确定性必须可见**：推断需要推理依据和其他可能解释，假设受到置信度上限约束。
- **时间比静态标签更重要**：矛盾不会被自动抹平，变化至少需要两个不同时间点。
- **用户拥有最终解释权**：AI 只生成 proposed 候选，用户审核状态优先于后续模型输出。
- **默认本地优先**：不自动上传文件、不开启遥测，真实远程模型调用必须由用户明确启用。

## 当前版本

[EchoMind 0.1.0](https://github.com/chanceline-max/EchoMind/releases/tag/v0.1.0) 是已经正式发布的本地优先 MVP。它支持通用 JSON、固定 CSV 和 TXT 导入，并完成导入、分析、证据审阅、Insight 修订和 EchoProfile 导出的本地闭环。

项目当前进入维护和真实使用反馈阶段。后续功能将根据实际使用结果、Issue 和可验证需求规划，而不是为了未来可能需要而提前扩张。

v0.1.0 不代表生产级稳定性、安全认证或正式隐私合规认证。Windows 11 已完成完整验证；Linux/macOS 尚未完成同等级验证。分析仍同步执行，SQLite 面向单工作区，真实远程 Provider 尚未广泛验证，WeFlow 也仍未支持。

## EchoMind 不是什么

- 不是聊天机器人。
- 不会用 MBTI、Big Five 或单一人格标签定义一个人。
- 不是医疗、心理诊断或治疗工具。
- 不把模型输出视为事实或不可修改的真相。
...
为减少逐条操作，审核页把 `proposed` 候选分成两个队列。最终置信度严格高于 50%、`evidence_state=valid` 且类型为 fact/preference/pattern/change 的候选进入“高置信快速确认”；inference、hypothesis、contradiction、证据非 valid 或置信度小于等于 50% 的候选仍需逐条审核。快速确认必须由用户点击并确认影响数量，不会由模型自动触发；每批最多 50 条，后端在同一事务中重新检查状态、版本、类型、证据和阈值，并为每条写入独立 Revision。任一条不符合或发生并发冲突时整批回滚。
...
EchoMind 使用 [Apache License 2.0](LICENSE)。版权与第三方归属信息见 [NOTICE](NOTICE)。
