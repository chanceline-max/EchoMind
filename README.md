...
当前正式发布版本为 [EchoMind 0.1.0](https://github.com/chanceline-max/EchoMind/releases/tag/v0.1.0)，已完成 Apache-2.0 开源发布。普通用户可以从本地界面完成导入→分析→审核→EchoProfile 闭环；审计证据和限制见 [MVP 审计报告](docs/MVP_AUDIT.md)。0.1.0 是供本地开发者试用的 MVP，API 和数据结构仍可能变化，不代表生产级稳定性、安全认证或正式隐私合规认证。
...
EchoMind v0.1.0 已正式发布：

- GitHub：[chanceline-max/EchoMind](https://github.com/chanceline-max/EchoMind)
- Release：[v0.1.0](https://github.com/chanceline-max/EchoMind/releases/tag/v0.1.0)
- License：[Apache-2.0](LICENSE)

当前项目进入维护和真实使用反馈阶段。后续功能将根据实际使用结果、Issue 和可验证需求进行规划。

v0.1.0 仍是本地优先的 MVP，不代表生产级稳定性或正式安全认证。当前主要限制包括同步分析、SQLite 单工作区、默认 Mock Provider 不生成真实候选、远程 Provider 兼容性尚未广泛验证，以及 WeFlow 尚未支持。
