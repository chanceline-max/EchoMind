...
当前已完成 **阶段 9：Insight 审核、乐观并发与修订历史**。本地审核台提供 Insight 列表/筛选/详情、匿名 Evidence、原消息定位、受限字段编辑、确认、驳回、恢复和 supersede。每次成功写入都要求 `expected_revision` 并追加 `InsightRevision`；消息排除或恢复会在同一事务内传播 Evidence 有效性、重算活动 Insight 的 `confidence-1.0` 并留下系统修订。当前仍没有 Profile 生成、用户自定义 confidence、批量审核或多用户权限。
...
阶段 9 已实现本地 Insight 审核 API/UI、Evidence 追溯、乐观并发、追加式 Revision 和消息排除传播；阶段 5 导入闭环及阶段 6–8 回归保持可用。真实 WeFlow 仍未支持；当前也没有分析/Provider/独立评分 API、模型设置、批量审核、用户 confidence override、跨窗口语义合并、EchoProfile、账户权限、Docker 或 CI。
