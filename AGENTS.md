...
* 默认不提供不可逆物理删除
* 优先使用归档、排除分析、软删除或状态标记
* 核心证据链外键禁止无提示级联删除
* 未来物理删除必须先计算并展示影响范围
* Evidence 失效后必须传播到 Insight 和 Profile，失去证据的结论不能继续显示为有效
...
* inference 必须保存推理依据和其他可能解释
* hypothesis 必须应用版本化置信度上限
* change 至少需要两个不同时间点
* contradiction 必须同时保留双方 Evidence
